...
export const createModelImageDataUrl = (row) => {
  ...
  const svg =
    ...
    `<stop offset="0%" stop-color="${gradientA}"/><stop offset="100%" stop-color="${gradientB}"/>` +
    `</linearGradient></defs>` +
    `<rect width="640" height="360" fill="url(#bg)"/>` +
    `<rect x="28" y="28" width="584" height="304" rx="26" fill="rgba(255,255,255,0.08)" stroke="rgba(255,255,255,0.28)"/>` +
    `<text x="320" y="166" fill="#ffffff" text-anchor="middle" font-size="76" font-family="${font}" font-weight="800">${escapeHtml(initials)}</text>` +
    `<text x="320" y="220" fill="#ffffff" text-anchor="middle" font-size="26" font-family="${font}" font-weight="700">${escapeHtml(label)}</text>` +
    `<text x="320" y="254" fill="rgba(255,255,255,0.9)" text-anchor="middle" font-size="18" font-family="${font}">${escapeHtml(manufacturer)}</text>` +
    `</svg>`;
